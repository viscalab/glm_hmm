...
DL TODO EL CÓDIGO LO QUE TIENE QUE VER CON EXTRAER LAS ESCALAS MEJOR COLOCALO EN OTRO ARCHIVO
...
DL. CÓDIGO ALTERNATIVO CON MÁS VERBOSIDAD PERO QUIZAS MÁS EXPLICITO
...
Supongo que el S1 es el disengaged (aunque está bastante sesgado a la derecha), mientras que el S2 es el engaged. 

DL. ESTRICTAMENTE NINGUNO DE LOS DOS ES DISENGAGED QUE ES UNA ETIQUETA QUE SE SUELE PONER PARA CURVAS MÁS PLANAS, PERO EFECTIVAMENTE EL ESTADO S1 ES MÁS IMPRECISO
...
```
Me parece muy extraño que el S1 sea predominante. Pienso que es posible que sea porque en realidad el S1 es la combinación de dos estados distintos, pero habrá que hacer la comparación de modelos.

YO CREO QUE NO ES TAN RARO QUE EL ESTADO MENOS PRECISO SEA DOMINANTE


AQUI COMO CALCULAR LA PREDOMINANCIA:
...
AÑADO UN 2 AL NOMBRE

OJO QUE LO QUE TIENES QUE DIBUJAR ES LA RESPUESTA, NO CORRECT. EL MODELO QUE HAS FITEADO ES LA PROPORCION DE VECES DE RESPONDER RIGHT, NO CORRECT

SE PUEDE VER CLARAMENTE COMO EN EL ESTADO 1 HAY UNA TENDENCIA A REPONDER MÁS DERECHA
...
AÑADO MAS VALORES INICIALES PARA QUE SE VEA MAS CLARAMENTE LA SATURACIÓN
...
No entiendo cómo interpretar esto o su utilidad (entiendo la utilidad del log_lik en comparación de modelos).
...
ENTRE TODOS LOS AJUSTES HAY QUE QUEDARSE CON EL QUE TIENE MÁS LIKELIHOOD:
...
AL FIT BEST (LE PUEDES LLAMAR glm_hmm2_s01 SIN USAR "_best" al final; lo he puesto para que se entienda). ESTE FIT ES EL QUE TIENES QUE UTILIZAR PARA TODOS LOS ANALISIS, POR EJ: 
...
VEO QUE AQUI TIENES CORRECTAMENTE Y = RESPONSE
...
AQUI TE FALTA LA PARTE DE COGER EL MEJOR FIT 



Comparación de 2 y 3 estados con el método Akaike: -2ln(L) + 2k (L = log_lik y k = n_par)
ES MEJOR EL MODELO DE DOS ESTADOS
```{r}
...
ggplot() +
  geom_line(data = glm_hmm3_10$posterior_probs, 
            aes(x = trial, y = p, color = state)) +
  geom_point(data = rand_10_sub_trials, aes(x = trial, y = response), shape = "|")
```
...
```{r}
glm_hmm2_10_AIC <- -2 * glm_hmm2_10$log_lik + 2 * glm_hmm2_10$n_par
glm_hmm3_10_AIC <- -2 * glm_hmm3_10$log_lik + 2 * glm_hmm3_10$n_par
...
ES MEJOR EL DE TRES ESTADOS
```{r}
glm_hmm2_30_AIC <- -2 * glm_hmm2_30$log_lik + 2 * glm_hmm2_30$n_par
glm_hmm3_30_AIC <- -2 * glm_hmm3_30$log_lik + 2 * glm_hmm3_30$n_par
