...
MEJOR LLAMAR A s01_trials ALGO QUE PONGA DESIGN MATRIX QUE ES LO QUE ES 
...
```
```{r}
glm_hmm2_s01 <- glm_hmm2_s01_seeds |> 
...
MEJOR HACER FUNCIONES PARA EL PLOT 
...
Esto lo tienes que arreglar para que tambien plotee las respuestas como antes

```{r}
plotting_trials_one_participant(glm_hmm2_s01)
```

Necistas instalar cowplot:
```{r}
...
Como tarda tanto habra que usar save para guardar los resultados
```{r}
#save(glm_hmm2, file = "RData/glm_hmm_all_seeds_2.RData")
```

y load cuando quieras cargarlos: 
