...
glm_hmm2_seeds_s01 |> 
  select(seed, fit) |> 
  unnest_wider(fit) |> 
  select(seed, recovered_weights) |> 
  unnest(recovered_weights) |> view()
  ggplot(aes(x = value)) +
...
glm_hmm2_all_seeds <- design_matrix_all |> 
  ...
  mutate(fit = list(glm_hmm_r(data, 
                              ...
                              .all_seeds = TRUE)))
```
Eliminé aquí la delcaración de la variable glm_hmm_extract best, porque ya está con las otras variables en la carpeta R
...
Con una muestra aleatoria de diez personas, parece que el modelo de 3S es superior
```{r}
