...
Psychometric plot for one participant:
```{r}
...
Trials plot for one participant:
```{r}
...
Psychometric and trials plot for one participant
```{r}
...
```{r}
glm_hmm2_all_seeds_2 <- design_matrix_all |> 
  group_by(subject) |> 
  nest() %>%
  rowwise() %>%
...
```{r}
glm_hmm_2_all <- glm_hmm2_all_seeds_2 %>% 
